...
function ensureSafeDatabase() {
  ...
  try {
    ...
    throw new Error(`clearDatabase aborted: DATABASE_URL is not a valid URL. Current DATABASE_URL=${raw}`);
  }
  ...
  if (!hostSafe || !dbSafe) {
    throw new Error(`clearDatabase aborted: DATABASE_URL=${raw} does not look like a test database`);
  }
}
...
export async function clearDatabase() {
  // Safety check: avoid accidental deletion on non-test DBs
  ensureSafeDatabase();
