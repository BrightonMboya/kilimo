      ...
      <div className="flex h-full flex-col justify-between">
        <ul className="menu">
          {menuItemsTop.map((item) => (
            <li key={item.label}>
              <Link
                className="group my-1 flex items-center gap-3 rounded px-3 py-2.5 text-[16px] font-semibold text-gray-700 transition-all duration-75 hover:bg-primary hover:text-white"
                href={item.to}
                title={item.label}
              >
                <i className="inline-flex pl-[2px] text-gray-500 transition duration-200 ease-linear group-hover:text-white">
                  {item.icon}
                </i>
