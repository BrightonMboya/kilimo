...
module.exports = async ({ github, context, core, headBranch, baseBranch }) => {
  try {
    ...
    try {
      ...
      const defaultBranch =
        repoResp && repoResp.data ? repoResp.data.default_branch : undefined;
      ...
      if (baseBranch) {
        try {
          ...
          core &&
            core.info &&
            core.info(
              `Requested base branch '${baseBranch}' not found in repo; falling back to default branch '${defaultBranch}'.`,
            );
      ...
      core &&
        core.info &&
        core.info(
          "Unable to fetch repository info; will attempt to use provided base branch.",
        );
    ...
    if (existing) {
      core &&
        core.info &&
        core.info(
          `Found existing PR #${existing.number} (${existing.html_url})`,
        );
