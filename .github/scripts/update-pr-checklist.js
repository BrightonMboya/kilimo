 ...
 */
module.exports = async ({ github, context, core, headBranch, runId, runUrl, conclusion }) => {
  try {
    const owner = context.repo.owner;
    const repo = context.repo.repo;

    // Find open PRs with this head
    const pulls = await github.rest.pulls.list({ owner, repo, state: 'open', head: `${owner}:${headBranch}` });
    if (!pulls.data || pulls.data.length === 0) {
      core && core.info && core.info(`No open PRs found for ${headBranch}; nothing to update.`);
      return null;
    }
    ...
    for (const job of jobs) {
      ...
      lines.push(`- [${ok ? 'x' : ' '}] ${job.name} — ${job.conclusion || job.status}`);
    }

    const failedJobs = jobs.filter(j => j.conclusion && j.conclusion !== 'success').map(j => j.name);

    if (failedJobs.length > 0) {
      lines.push('\n**Failed jobs:** ' + failedJobs.join(', '));
      lines.push('\nLogs: ' + runUrl + ' (use the Jobs tab and click a failing job to view detailed logs)');
    }
    ...
    if (body.includes(start)) {
      body = body.replace(new RegExp(`${start}[\s\S]*?${end}`,'m'), newSection);
    } else {
      body = newSection + '\n\n' + body;
    }

    await github.rest.pulls.update({ owner, repo, pull_number: pr.number, body });
    core && core.info && core.info(`Updated PR #${pr.number} with CI checklist.`);
    return pr.number;
  } catch (err) {
    console.error('update-pr-checklist failed:', err && err.message ? err.message : err);
    throw err;
  }
};
