...
module.exports = async ({ github, context, needs }) => {
  try {
    ...
    const jobDetails = {
      ...
      "test-runner":
        needs && needs["test-runner"] ? needs["test-runner"].result : "skipped",
    ...
    const priorityLabel =
      criticalFailures.length > 0 ? "HIGH PRIORITY" : "MEDIUM PRIORITY";
...
${issueBody}`,
      ...
      const finalLabels = Array.from(
        new Set(labels.map((l) => String(l).trim())),
      );
